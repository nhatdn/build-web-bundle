...
import {
    ReactComponent as IconBars
} from '../assets/menu-top/bars-solid.svg';
import {
    ReactComponent as IconClose
} from '../assets/menu-top/xmark-solid.svg';
import {
    ReactComponent as IconUp
} from '../assets/flag/angle-down-solid.svg';
...
const MenuComponent = () => {
        ...
        return ( <
            ...
            alt = "Marketplace"
            className = "w-[120px] md:w-[65%] mt-[11px]" / >
            <
            /a>
            ...
            <
            IconBars className = "w-6 md:w-12 fill-white"
            onClick = {
                () => {
            ...
            /> <
            /div> <
            /div> { /* ${visible ? 'right-0' : 'right-[-1000px]'} */ } {
                visible && ( <
                    div onClick = {
                        () => setVisible((prev) => !prev)
                    }
                    className = {
                        `w-full h-screen bg-black/70 fixed top-0 right-0 flex justify-end  `
                    } >
                    < /div>
                )
            } <
            div className = {
                `px-10 w-4/6 h-screen p-4 bg-[#A62F26] absolute top-0 ${
          visible ? 'right-0' : 'right-[-1000px]'
            ...
            <
            IconClose className = "w-4 md:w-8 fill-white"
            onClick = {
                () => {
                    setVisible((prev) => !prev);
                }
            ...
            ul > {
                MENU.map((menu, key) => {
                        return ( <
                            ...
                            className = "border-b border-b-[#C65E56]" >
                            <
                            a href = {
                                menu.href
                            }
                            ...
                            } >
                            {
                                ...
                                menu ? .children ? .length > 0 && ( <
                                    IconUp className = {
                                        `inline-block h-5 ml-2 transition ${expands.includes(menu.name) && 'rotate-180'}`
                                    }
                                    />
                                )
                            } <
                                            ...
                                            menu ? .children ? .map((submenu, key) => ( <
                                                    li key = {
                                                        ...
                                                        submenu.href === 'coming-soon' ? ( <
                                                            Popup modal trigger = { <
                                                                a className = {
                                                                    `font-bold text-[16px] block text-white py-1`
                                                                } > {
