...
const ItemSlideNews = ({ href, img, title, content }) => {
  return (
    <div className="px-3 text-white font-Fira-Sans-italic">
      <a href={href} rel="noopener noreferrer" target={'_blank'}>
        <img src={img} alt="" className="rounded-lg h-[143px] w-full m-auto object-cover object-center" />
        <p className="py-4 text-xl font-medium">{title}</p>
        <p className="pb-4 text-base ">{content}</p>
      </a>
    </div>
  );
};

const ItemEventNew = ({ href, img, title, content }) => {
  return (
    <div className="p-8 md:flex">
      <div className="md:w-2/6">
        <img src={img} alt="" className="w-full m-auto rounded-xl" />
      </div>
      <div className="md:w-4/6 md:pl-4">
        <a href={href} rel="noopener noreferrer" target={'_blank'}>
          <p className="my-3 text-xl font-medium">{title}</p>
          <p className="mt-2 text-base ">{translate(content)}</p>
        </a>
      </div>
    </div>
  );
  ...
  return (
    <>
      <div className="w-full px-5 pt-5">
        <ItemEventNew
          key={ArrayLastestNews[0].id}
          href={ArrayLastestNews[0].href}
          img={ArrayLastestNews[0].image}
          title={ArrayLastestNews[0].title}
          content={ArrayLastestNews[0].content}
        />
        <div className="w-full h-[2px] bg-[#FF9593]/40 px-8"></div>
        <ItemEventNew
          key={ArrayLastestNews[1].id}
          href={ArrayLastestNews[1].href}
          img={ArrayLastestNews[1].image}
          title={ArrayLastestNews[1].title}
          content={ArrayLastestNews[1].content}
        />
      </div>
...
const LastestNewsComponent = () => {
  return (
    <div id="news" className="w-full text-white bg-no-repeat bg-cover bg-mb-lastest-news font-Fira-Sans-italic">
