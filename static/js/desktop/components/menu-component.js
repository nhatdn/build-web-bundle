    ...
    <div className="fixed top-0 w-full left-0 z-[999] px-4 xl:px-10 2xl:px-[100px] h-[100px]  2xl:h-[120px]">
      <div className="flex items-center h-full gap-1">
        ...
        <ul className="flex items-center font-bold text-white min-w-[840px] w-[78%] xl:w-[75%]  2xl:pl-[70px]">
          {MENU.map((menu, index) => {
            return (
              <li key={menu + index} className="relative inline-block group">
                <a
                  // onClick={() => setMenuActive(menu.state)}
                  href={menu.href}
                  target={menu.state === 4 ? '_blank' : '_parent'}
                  className={`text-sm 2xl:text-[16px] hover:bg-[#8A0D14]  transition-colors duration-500 ease-in-out block px-2 3xl:px-4 leading-[50px] ${
                ...
                >
                  {translate(`${menu.name}`)}
                  {menu?.children?.length > 0 && <MenuChild submenu={menu?.children} />}
                </a>
              </li>
            );
          })}
